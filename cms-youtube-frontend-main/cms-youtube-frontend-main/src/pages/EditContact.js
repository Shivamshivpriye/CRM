...
const EditContact = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api0/contact/${id}`, {
        method: "PUT",
        headers: {
      ...
      if (!result.error) {
        ...
        setUserDetails({
          ...
          products: "",
          requirement: "",
          status: "", // Reset process field after submission
          query:"",
        });
        navigate("/mycontacts");
      } else {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setUserDetails({
          ...
          status: result.status, // Set status field from fetched data
          query:result.query,
        });
        setLoading(false);
      } catch (err) {
